...
#added by ubaid
tf.app.flags.DEFINE_string(
    'log_fn_pref', None, 'Path prefix of the log path where org_graph, timeline and metadata are stored.')

tf.app.flags.DEFINE_boolean(
    'doLog', True, 'set if we want to enable or disable logging while running the experiment')
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with open('%s/org_train_graph_notensors.json' % (FLAGS.log_fn_pref), 'w') as outfile:
      json.dump(adj_list_graph_notensors, outfile)

    adj_list_graph = {}
    for op in all_ops:
      adj_list_graph[op.name] = set([inp.name for inp in op.inputs])
    adj_list_graph = {op_name:list(op_deps) for op_name, op_deps in adj_list_graph.items()}
    with open('%s/org_train_graph.json' % (FLAGS.log_fn_pref), 'w') as outfile:
